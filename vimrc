...
noremap <silent> <C-i> :echo "Please wait, removing subdomains..."<CR>:GitGutterDisable<CR>:%!f=$(mktemp); cat - > $f;grep -vxf <(comm -23 <(sort $f) <(sort $f \| uniq \| rev \| sort \| awk 'NR\!=1&&substr($0,0,length(p))==p{next}{p=$0".";print}' \| rev\|sort)) $f; rm -f $f<CR>:$<CR>:GitGutterEnable<CR>:echo ""<CR>
